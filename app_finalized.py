...
region = "us-east-1"
llm_endpoint_name = "meta-textgeneration-llama-3-8b-instruct-2024-11-13-02-30-41-071"
embedding_endpoint_name = "hf-sentencesimilarity-bge-large-en-v1-5-2024-11-13-02-39-44-311"
...
class StableDiffusionContentHandler:
    ...
    def transform_input(self, prompt: str) -> bytes:
        ...
        payload = {
            "prompt": prompt, 
            "height": 512,  # Adjust the height
            "width": 512,   # Adjust the width
    ...
    def transform_output(self, output: bytes) -> Image:
        try:
            response_json = json.loads(output.decode("utf-8"))
            logger.info(f"Response from endpoint: {response_json}")

            # Check if the 'image' key exists in the response
            if "image" not in response_json:
                logger.error("Missing 'image' key in response")
                return None
...
def generate_image_from_title(title: str) -> List[str]:
    ...
    try:
        ...
        image = sd_content_handler.transform_output(response['Body'].read())

        # Save the image as a PNG file locally
